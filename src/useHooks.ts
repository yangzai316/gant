...
export interface UseDateInfoType {
  total: number;
  currentMonthFirstWeek: number;
  today: number;
  toHours: string;
  toMinutes: string;
...
export const useDateInfo: (year: number, month: number) => UseDateInfoType = (
  year,
  month
) => {
  return useMemo(() => {
    ...
    const total = getMonthDays(year, month) as number;
    const currentMonthFirstDay = +new Date(`${year}-${month}-01 00:00:00`);
    const currentMonthFirstWeek = new Date(
